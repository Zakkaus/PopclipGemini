...
async function resolveAuth(configStr, locationOverride, backendOverride) {
  ...
  let cfg;
  try { cfg = JSON.parse(configStr); } catch (_) {
  ...
  if (cfg.type === "service_account" && cfg.private_key) {
    ...
    return { backend: "vertex-ai", project: cfg.project_id, location: defaultLocation, accessToken: token };
  }
  if (cfg.project_id) {
    return { backend: "vertex-ai", project: cfg.project_id, location: cfg.location || defaultLocation, apiKey: cfg.api_key || null, accessToken: cfg.access_token || null };
  }
  return { backend: "google-ai", apiKey: configStr.trim() };
}
...
function buildHeaders(auth) {
  ...
  if (auth.backend === "vertex-ai" && auth.accessToken) h["Authorization"] = `Bearer ${auth.accessToken}`;
  return h;
}
...
async function callGemini(prompt, configStr, model, locationOverride, backendOverride) {
  ...
  const body = {
    ...
    generationConfig: { temperature: 1.0, maxOutputTokens: 8192, topP: 0.95, topK: 64 },
  };
  try {
    const res = await axios.post(buildUrl(auth, model), body, { headers: buildHeaders(auth) });
